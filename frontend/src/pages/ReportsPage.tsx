  ...
  return (
    <section className="min-w-0 rounded-3xl border border-white/10 bg-panel p-4 sm:p-6">
      ...
      <div className="mt-6 space-y-3">
        {reports.length === 0 && (
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-mute">
            暂无历史报告。运行一次真实分析后，这里会出现报告列表。
          </div>
        )}
        {reports.map((report) => (
          <article key={report.id} className="flex flex-col gap-4 rounded-2xl border border-white/10 bg-white/5 p-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => handleExport(report.id)} className="rounded-2xl border border-white/10 bg-panel2 px-4 py-2 text-sm hover:border-cyanx/50">导出 HTML</button>
            </div>
          </article>
        ))}
      </div>
